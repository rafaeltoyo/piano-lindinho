...
class MotionDetector:
    ...
    def detect_white_key_motion(self, current_frame, previous_frame, current_hand_mask, previous_hand_mask,
        ...
        if not is_synthesia:
            ...
            cv2.imshow('Absolute Difference without HANDS', frame_diff)
            frame_diff[hand_region == 0] = 0
            frame_diff = cv2.normalize(frame_diff, None,alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            cv2.imshow('Absolute Difference only in hand_regions', frame_diff)
            frame_diff[shadow_mask > 0] = 0
            frame_diff = cv2.normalize(frame_diff, None,alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            cv2.imshow('Absolute Difference only in hand_regions', frame_diff)
            cv2.imshow('hand', shadow_mask)

        frame_diff[self.threshold_image > 0] = frame_diff[self.threshold_image > 0] * 5
        frame_diff[self.threshold_image == 0] = 0
        frame_diff = cv2.normalize(frame_diff, None,alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        cv2.imshow('Absolute Difference only in white keys', frame_diff)
        frame_diff = cv2.blur(frame_diff, (3,frame_diff.shape[0]))
        frame_diff = cv2.normalize(frame_diff, None,alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        cv2.imshow('Absolute Difference spread out across a key', frame_diff)
    ...
    def detect_key_stroke(self, current_frame, previous_frame, is_synthesia):
        ...
        for kernel_size in [11]:
            ...
            total_choripan = cv2.normalize(total_diff, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            cv2.imshow('Motion after horizontal Kernel', total_choripan)
